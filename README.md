...
Product behavior lives under `src/modules/<module>`. The todo example is shaped as a production-style vertical slice with NestJS-inspired module roles: module, controller, providers, service, repository, DTO, mapper, and types. Cross-module stateless helpers live under `src/libraries`, while stateful infrastructure integrations stay under `src/services`.

Backend module conventions are documented in [`docs/backend-modules.md`](docs/backend-modules.md).
...
`better-auth` is mounted directly on the Elysia app, and authenticated todo mutations resolve the current session through the shared auth guard. Drizzle owns the PostgreSQL schema and migration workflow.
